...
class Pipeline:
    def __init__(
        ...
        # We'll store the top/left offsets from detect_edges for row->mm_x mapping
        self.crop_left = 0
        self.crop_top = 0
    ...
    def convert_to_mm(self, pixel_coords, conversion_factor):
        ...
        for (r, c) in pixel_coords:
            # Original mapping with 90° rotation: row → mm_x, column → mm_y
    ...
    def convert_to_robot_coordinates(self, mm_coords):
        ...
        for (mm_x, mm_y) in mm_coords:
            ...
            z_robot = self.plate_origin_in_robot[2]  # keep pipeline's Z
